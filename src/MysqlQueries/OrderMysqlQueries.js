...
objOrder.CreateOrder = async (objOrder) => {
  try {
    const { strDocumentCustomer, strDocumentSeller } = objOrder;
    let strDataOrder = null;
    //Connection
    let Connection = await objMysql.MysqlConnection();
    await new Promise((resolve, reject) => {
      Connection.query(
        `CALL SP_CreateOrder('${strDocumentSeller}','${strDocumentCustomer}')`,
        (err, rows) => {
          if (err) {
            console.log(err);
            reject(err);
...
objOrder.FinalizeOrder = async (objOrder) => {
  try {
    const { intIdOrder } = objOrder;
    //Connection
    let Connection = await objMysql.MysqlConnection();
    await new Promise((resolve, reject) => {
      Connection.query(`CALL SP_FinalOrder('${intIdOrder}')`, (err, rows) => {
        if (err) {
          console.log(err);
          reject(err);
        }
...
objOrder.CreateDetail = async (objOrderDetail) => {
  try {
    ...
    await new Promise((resolve, reject) => {
      Connection.query(
        `CALL SP_CreateDetailOrder('${intQuantityOrder}','${intTotalOrder}',
