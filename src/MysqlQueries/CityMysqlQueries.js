...
objCityMysqlQueries.CreateCity = async (objCity) => {
  try {
    const { strDescription, intIdZone } = objCity;
    //Conection Mysql
    ...
    await new Promise((resolve, reject) => {
      Connection.query(
        `
                CALL SP_CreateCity('${strDescription}','${intIdZone}')`,
        (err, rows) => {
          //Error
          if (err) {
...
objCityMysqlQueries.EditCity = async (objCity) => {
  try {
    const { intIdCity, strDescription, intIdZone } = objCity;
    //Conection Mysql
    let Connection = await objMysql.MysqlConnection();
    ...
    await new Promise((resolve, reject) => {
      Connection.query(
        `
                CALL SP_EditCity('${intIdCity}','${strDescription}','${intIdZone}')`,
        (err, rows) => {
          //Error
          if (err) {
            blnEstadoQuery = false;
